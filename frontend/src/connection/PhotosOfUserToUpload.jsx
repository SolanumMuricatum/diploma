...
export const PhotosOfUserToUpload = ({ slots, setSlots }) => {
  ...
  const postPhoto = async (base64String) => {
    try {
      const response = await fetch(`http://localhost:8087/album/save/photo`, {
        method: 'POST',
        credentials: 'include',
        headers: {
  ...
  const deletePhoto = async (photoId) => {
    try {
      const response = await fetch(
        `http://localhost:8087/album/delete/photo?photoId=${photoId}`,
        {
          method: 'DELETE',
          credentials: 'include',
        }
  ...
  return (
    <div
      className="main-photos-to-upload-container"
          ...
          <div key={index} className="photos-to-upload-photo-container">
            {slot ? (
              <>
                ...
                <img
                  src={slot.preview || slot.urlFromServer}
                  alt="user"
                  style={{
                    width: '100%',
                    height: '100%',
