...
export const Account = () => {
  ...
  const handleUpdateUserLogin = async (field, value) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8087/user/account/login/update`,
        {
          method: 'PUT',
      ...
      if (response.ok) {
        toast.success('Данные обновлены! Пожалуйста, авторизуйтесь повторно.', {
          onClose: () => {
            navigate(`/login`);
  ...
  const handleUpdateUserEmail = async (field, value) => {
    ...
    if (!emailRegex.test(trimmedEmail)) {
      toast.error(
        'Пожалуйста, введите корректный адрес электронной почты (например, user@mail.com)'
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `http://localhost:8087/user/account/email/update`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
  ...
  const postPhoto = async (base64String) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8087/user/account/save/photo`,
        {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
      ...
      toast.error('Произошла ошибка при сохранении фото');
    }
  };
  ...
  const handleDeletePhoto = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8087/user/account/delete/photo?userId=${userId}`,
        {
          method: 'DELETE',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
  ...
  const submitPasswordChange = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8087/user/account/password/update`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
      ...
      if (response.ok) {
        ...
        toast.error(errorData.error || 'Ошибка при смене пароля');
      }
    } catch (e) {
      toast.error(e);
      console.error(e);
  ...
  const handleDeleteAccount = async () => {
    ...
    if (password !== null) {
      if (password && password.trim() !== '') {
        try {
          console.log('Отправляем запрос на удаление для ID:', userId);

          const response = await fetch(`http://localhost:8087/user/delete`, {
            method: 'DELETE',
            credentials: 'include',
            headers: {
        ...
        toast.info('Пожалуйста, введите пароль для подтверждения удаления.');
      }
    }
  };
  ...
  return (
    <div className="account-container">
      ...
      <div className="account-data-container">
        <div
          className="account-photo-container"
        ...
        >
          ...
          {isMenuOpen && (
            <div
              className="account-photo-options"
              ref={menuRef}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="menu-item" onClick={handleAddPhotoClick}>
                Добавить фото
              </div>
              <div className="menu-item" onClick={handleDeletePhoto}>
                Удалить фото
              </div>
            </div>
          )}
