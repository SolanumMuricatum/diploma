...
export const Login = () => {
  ...
  const postUser = async () => {
    try {
      const data = await apiFetch(`http://localhost:8087/user/save`, {
        method: 'POST',
  ...
  const handlePasswordReset = async (e) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8087/user/login/data/recovery?email=${resetEmail}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        }
  ...
  return (
    <div className="container">
      ...
      {activeTab === 'login' && (
        ...
        >
          ...
          <div className="options">
            <a href="#!" onClick={() => setIsForgotModalOpen(true)}>
      ...
      {isForgotModalOpen && (
        ...
        >
          <div
            className={`recovery-modal-content ${isClosing ? 'closing' : ''}`}
