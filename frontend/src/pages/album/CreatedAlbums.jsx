...
export function CreatedAlbums() {
    ...
    const fetchEvent = async () => {
      try {
        let response = await fetch(
          `http://localhost:8087/album/get/all/created?creatorId=${userId}`,
          {
            method: 'GET',
  ...
  const handleDeleteAlbum = async (albumId) => {
    const isConfirmed = window.confirm(
      'Вы уверены, что хотите удалить фотоальбом?'
    );
    ...
    try {
      const response = await fetch(`http://localhost:8087/album/delete`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {
      ...
      if (response.ok) {
        toast.success('Вы успешно удалили фотоальбом!', {
          onClose: () => {
      ...
      <div className="created-albums-container">
        ...
        <div className="created-albums-wrapper">
          ...
          {albums && albums.length === 0 && (
            <div>У вас пока нет созданных альбомов.</div>
          )}
                  ...
                  <div className="created-album-card-wrapper">
                    <div className="created-album-card">
                      ...
                      <div style={{ position: 'relative' }}>
                        ...
                        <div
                          className="created-album-options-icon"
                          onClick={(e) => {
                          ...
                          >
                            ...
                            <div
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                navigate(
                                  `/albums/created/manage/photos/${album.id}`
                                );
                              }}
                            >
                            ...
                            <div
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                navigate(
                                  `/albums/created/manage/members/${album.id}`
                                );
                              }}
                            >
                              Управлять доступом
