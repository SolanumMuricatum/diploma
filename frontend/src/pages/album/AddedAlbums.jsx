...
export function AddedAlbums() {
    ...
    const fetchEvent = async () => {
      try {
        let response = await fetch(
          `http://localhost:8087/album/get/all/added?userId=${userId}`,
          {
            method: 'GET',
  ...
  const handleLeaveAlbum = async (albumId) => {
    const isConfirmed = window.confirm(
      'Вы уверены, что хотите покинуть фотоальбом?'
    );
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          'Вы успешно покинули фотоальбом! Все Ваши фото были удалены.',
          {
            onClose: () => {
      ...
      <div className="added-albums-container">
        ...
        <div className="added-albums-wrapper">
          ...
          {albums && albums.length === 0 && (
            <div>У вас пока нет добавленных альбомов.</div>
          )}
                  ...
                  <div className="added-album-card-wrapper">
                    <div className="added-album-card">
                      ...
                      <div style={{ position: 'relative' }}>
                        ...
                        <div
                          className="added-album-options-icon"
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                          ...
                          >
                            <div
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                navigate(
                                  `/albums/added/manage/photos/${album.id}`
                                );
                              }}
                            >
