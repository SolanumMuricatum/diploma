...
export function CreateUpdateAlbum() {
      ...
      const getAlbum = async () => {
        try {
          let response = await fetch(
            `http://localhost:8087/album?id=${albumId}`,
            {
              method: 'GET',
  ...
  const postAlbum = async () => {
    if (!album.name.trim()) {
      toast.error('Введите название фотоальбома!');
      return;
    }
    try {
      const response = await fetch(`http://localhost:8087/album/save`, {
        method: 'POST',
        credentials: 'include',
        headers: {
        ...
        body: JSON.stringify({
          ...
          textFont: album.textFont,
          textSize: album.textSize,
          creatorId: userId,
          creatorLoginSnapshot: userLogin,
        }),
      });
  ...
  const putAlbum = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8087/album/update?albumId=${albumId}`,
        {
          method: 'PUT',
          credentials: 'include',
          headers: {
      ...
      if (statusCode != 200) {
        ...
        toast.success('Фотоальбом успешно отредактирован!', {
          onClose: () => {
            window.location.reload();
          },
        });
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {err && (
        <div>
          <div className="album-create-title">Ошибка доступа к мероприятию</div>
        </div>
      )}
      {!err && (
        <div>
          <div
            ...
            style={{
              ...
              fontFamily: `${album.textFont === 'None' ? `-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji"` : `${album.textFont}`}`,
              fontSize: `${album.textSize}pt`,
            }}
          >
            {album.name}
          </div>
            ...
            <div className="text-font-create-album">
              {textFonts.map((font) => (
                <div>
                  {album.textFont === font && (
                    <input
                      style={{ accentColor: '#112250' }}
                      type="radio"
                      id={font}
                      name="font"
                      value={font}
                      defaultChecked
                      onChange={() => handleTextFontCLick(font)}
                    />
                  )}
                  {album.textFont != font && (
                    <input
        ...
        <div className="third-block-create-album">
          <div className="text-size-create-album">
            ...
            <input
              ...
              min="28"
              max="60"
              defaultValue={album.textSize}
              onChange={(e) => handletextSizeClick(e.target.value)}
            />
          </div>
          {!err && (
