...
export function CurrentAlbum() {
  ...
  const showPrev = (e) => {
    e.stopPropagation();
    setCurrentPhotoIdx(
      (prev) => (prev - 1 + allPhotosFlat.length) % allPhotosFlat.length
    );
  };
    ...
    const fetchAllData = async () => {
      try {
        const res = await fetch(
          `http://localhost:8087/album/get/all/members?albumId=${albumId}`,
          {
            credentials: 'include',
          }
        );
  ...
  const fetchUserPhotos = async (userId) => {
    try {
      const res = await fetch(
        `http://localhost:8087/album/get/all/photo?albumId=${albumId}&userId=${userId}`,
        {
          credentials: 'include',
        }
      );
  ...
  const handleToggleAllPhotos = () => {
    ...
    const isAllSelected =
      allLoadedIds.length > 0 && selectedPhotos.size === allLoadedIds.length;
  ...
  const handleDownloadSelected = async () => {
    ...
    try {
      const downloadPromises = photosToDownload.map(async (photo) => {
        ...
        const fileName =
          photo.image.split('/').pop() || `photo_${photo.id}.jpg`;
      ...
      toast.error('Не удалось скачать некоторые фото :(');
    } finally {
  ...
  }, [currentPhotoIdx, allPhotosFlat.length]);
    ...
    <div style={{ position: 'relative' }}>
      ...
      <div className="album-control-panel">
        ...
        <div className="album-control-panel-options">
          <button onClick={handleToggleAllPhotos}>Выделить всё</button>
          <button
      ...
      {members.map((member) => (
        <div key={member.id} className="member-block">
          <div className="friend-data-container-wrapper">
            ...
            <div className="friend-select-all-container">
              ...
              <input
                ...
                id={`select-${member.id}`}
                checked={(() => {
                  const photos = (photosByUser[member.id] || []).filter(
                    (p) => p !== null
                  );
                  return (
                    photos.length > 0 &&
                    photos.every((p) => selectedPhotos.has(p.id))
                  );
                })()}
          ...
          <div className="friend-photo-container">
            {(photosByUser[member.id] || Array(10).fill(null)).map(
              (photo, idx) => {
                ...
                return (
                  <div
                    ...
                    onDoubleClick={() => {
                      if (photo) {
                        const index = allPhotosFlat.findIndex(
                          (p) => p.id === photo.id
                        );
      ...
      {currentPhotoIdx !== null && allPhotosFlat[currentPhotoIdx] && (
        ...
        >
          ...
          >
            <img src={allPhotosFlat[currentPhotoIdx].image} alt="Full size" />
          </div>
