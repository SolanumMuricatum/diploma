...
export const AlbumInvitationsHistory = () => {
    ...
    const fetchData = async () => {
      try {
        const invRes = await fetch(
          `http://localhost:8087/album/get/all/album/invitations?albumId=${albumId}`,
          {
            credentials: 'include',
          }
        );
        ...
        const userRes = await fetch(
          `http://localhost:8087/user/getAll?ids=${ids}`,
          {
            credentials: 'include',
          }
        );
  ...
  const handleRetryInvitation = async (targetUser) => {
    const isConfirmed = window.confirm(
      `Вы уверены, что хотите повторно пригласить ${targetUser.login}?`
    );
    ...
    try {
      const res = await fetch(`http://localhost:8087/album/invitation/retry`, {
        method: 'POST',
        credentials: 'include',
