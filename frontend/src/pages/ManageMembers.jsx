...
export const ManageMembers = () => {
    ...
    const fetchAllData = async () => {
      try {
        const res = await fetch(
          `http://localhost:8087/album/get/members?albumId=${albumId}`,
          {
            credentials: 'include',
          }
        );
  ...
  const handleDeleteAlbumMember = async (userId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8087/album/added/leave`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {
      ...
      if (response.ok) {
        toast.success(
          'Вы успешно удалили участника! Все его фото были удалены.',
          {
            onClose: () => {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      if (searchQuery.length >= 2) {
        // Начинаем искать от 2-х символов
  ...
  const fetchUsers = async (query) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8087/user/search?query=${query}&albumId=${albumId}`,
        {
          method: 'GET',
          credentials: 'include',
          headers: {
  ...
  const handleInvite = async (targetUser) => {
    const isConfirmed = window.confirm(
      `Вы уверены, что хотите пригласить ${targetUser.login}?`
    );
    ...
    try {
      const res = await fetch(`http://localhost:8087/album/save/invitation`, {
        method: 'POST',
        credentials: 'include',
    ...
    <div className="main-manage-members-container">
      ...
      <div className="album-control-panel">
        ...
        <div className="album-control-panel-options">
          ...
          <button
            onClick={() =>
              navigate(`/albums/${parent}/members/history/${albumId}`)
            }
          >
            История заявок
          </button>
      ...
      {isModalOpen && (
        <div className="modal-overlay" onClick={() => setIsModalOpen(false)}>
          ...
          >
            ...
            <h3 style={{ color: '#112250', marginBottom: '20px' }}>
              Поиск пользователей
            </h3>
            ...
            <div className="search-results-list">
              {isSearching && (
                <div style={{ textAlign: 'center' }}>Ищем...</div>
              )}
